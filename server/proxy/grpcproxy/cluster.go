...
type clusterProxy struct {
	...
	umu  sync.RWMutex
	umap map[string]naming.NamingUpdate
}
...
func NewClusterProxy(lg *zap.Logger, c *clientv3.Client, advaddr string, prefix string) (pb.ClusterServer, <-chan struct{}) {
	...
	cp := &clusterProxy{
		...
		advaddr: advaddr,
		prefix:  prefix,
		umap:    make(map[string]naming.NamingUpdate),
	}
...
func (cp *clusterProxy) monitor(wa naming.NamingWatcher) {
	for cp.ctx.Err() == nil {
		ups, err := wa.Next()
		if err != nil {
		...
		for i := range ups {
			switch ups[i].Op {
			case naming.NamingAdd:
				cp.umap[ups[i].Addr] = *ups[i]
			case naming.NamingDelete:
