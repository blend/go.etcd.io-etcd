...
func TestGRPCResolver(t *testing.T) {
	...
	addOp := namingv3.NamingUpdate{Op: namingv3.NamingAdd, Addr: "127.0.0.1", Metadata: "metadata"}
	...
	wu := &namingv3.NamingUpdate{
		Op:       namingv3.NamingAdd,
		Addr:     "127.0.0.1",
		Metadata: "metadata",
	}
	...
	delOp := namingv3.NamingUpdate{Op: namingv3.NamingDelete, Addr: "127.0.0.1"}
...
func TestGRPCResolverMulti(t *testing.T) {
	...
	v, verr := json.Marshal(namingv3.NamingUpdate{Addr: "127.0.0.1", Metadata: "md"})
	if verr != nil {
		t.Fatal(verr)
	}
	...
	if len(updates) != 2 || (updates[0].Op != namingv3.NamingDelete && updates[1].Op != namingv3.NamingDelete) {
		t.Fatalf("expected two updates, got %+v", updates)
	}
}
